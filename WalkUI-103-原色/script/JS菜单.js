...
const main_menu = () => {
    if (JS_list.length === 0) {
        clientMessage('§b§e读取脚本列表失败(可能是没有储存权限)');
        return;
    }
    var menu = {
        type: 'form',
    ...
    addForm(JSON.stringify(menu), function(index) {
        ...
        if (index >= 2) {
            ...
            load(js_name)
            clientMessage('§b§e加载脚本 => §b' + js_name)
...
const settings = () => {
    ...
    addForm(json, function(delay, times, mode, e, debug) {
        config.delay = delay
        config.times = times
...
const load = (script_name) => {
    ...
    var state = false
    setData("script", script_name)
    setTimeout(() => {
        for (let i = 0; i < config.times; i++) {
            if (config.mode === "eval") {
                var js = js_text + ((config.allowExit && js_text.indexOf("exit()") === -1) ? "\nfunction onSendChatMessageEvent(text) {if(text === \"退出\") {clientMessage('§b§e已退出'); exit(); return true;}}" : "");
                state = eval((config.debug) ? "try {\n" + js + "\n}catch(e){clientMessage('§b§e发现错误 => '+e.stack)};" : js);
            }
            if (config.mode === "load") {
                ...
                if (config.debug && js_text.indexOf("try") === -1) {
                    write_file(js_path, "try {\n" + js_text + "\n}catch(e){clientMessage('§b§e发现错误 => '+e.stack)};");
                    js_text = read_file(js_path);
                }
                state = loadScript(script_name);
            }
        }
        clientMessage('§b§eJS加载' + (state ? '成功' : '失败'))
    }, config.delay * 1000);
};
